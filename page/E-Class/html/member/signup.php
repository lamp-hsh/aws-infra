<?php include "../../inc/eclassinfo.inc";
    ...
    if(!is_null($name) && !is_null($birth) && ($stud_id !== '' || $prof_id !== '')) {
        ...
        if(!$row[$column]) echo $query;
        //echo $row[0]; //echo $column;//echo("<script>alert('인증에 실패했습니다.'); history.back();</script>");
        $tab_mem = $tab . '_mem';
        $query2 = "SELECT * FROM $tab_mem WHERE $column2 = '$col_id';";
        $select2 = mysqli_query($connection, $query2);
        $row2 = mysqli_fetch_array($select2);
        if($row2['id'] != '') echo "<script>alert('이미 회원가입을 하셨습니다.'); location.href='/main.php';</script>";
    }
    else {
...
<html>
    ...
    <body>
        ...
        <form action="signup_proc.php" method="POST">
            ...
            <div class="int-area">
            <?php
                $stud_prof_id = $row[$column];
                echo "<input id='stud_prof_id' type='text' name='stud_prof_id' value='$stud_prof_id'><label for='stud_prof_id'>$text_id</label>";
