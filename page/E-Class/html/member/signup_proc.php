<?php include "../../inc/eclassinfo.inc";
    ...
    if(!is_null($stud_prof_id) && !is_null($id) && !is_null($pw) && !is_null($pw_check) && !is_null($flag)) {
        if($pw != $pw_check) echo "<script>alert('비밀번호 확인 실패'); history.back();</script>";
        else{
            ...
            if($flag === "professor") {
                ...
                $query2 = "INSERT INTO stud_mem (stud_id, id, pw) VALUES ('$stud_prof_id', '$id', '$pw');";
            }
            $select = mysqli_query($connection, $query);
            while($row = mysqli_fetch_array($select)) {
                $id_check = $row['id'];
            }
            if($id == $id_check) echo '<script>alert("ID 중복"); history.back();</script>';
